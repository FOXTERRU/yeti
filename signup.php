...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    if(!empty($name)&&!empty($contacts)&&!empty($email)&&!empty($pass)&&!empty($avatar))
    {
        ...
        if (mysqli_num_rows($result) > 0)
        {
            ...
            $page_content = include_template('signup.php', $data_main);
        ...
        {
            $query = "INSERT INTO users (id_user,registration_date,email,name,pass,avatar,contacts) VALUES (NULL,now(),'$email','$name','$pass','$avatar','$contacts')";
